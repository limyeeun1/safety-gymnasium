...
class Agent:
    ...
    def choose_action(self, state, goal, train_mode=True):
        ...
        with torch.no_grad():
            ...
            # action[0] = action[0] * self.action_bounds[1][0]
            # action[1] = action[1] * self.action_bounds[1][1]
    ...
    def train(self):
        ...
        # print("states before normalize", states[0,:])
        states = self.state_normalizer.normalize(states)
        # print("states after normalize",states[0,:])
        ...
        with torch.no_grad():
            ...
            # print("target_return",target_returns[:10,:]," -> ",torch.clamp(target_returns[:10,:], -1 / (1 - self.gamma), 0))
            target_returns = torch.clamp(target_returns, -1 / (1 - self.gamma), 0)


        q_eval = self.critic(inputs, actions)
        # print("q_eval",q_eval[:10,:])
    ...
    def save_weights(self):
        torch.save({"actor_state_dict": self.actor.state_dict(),
                    ...
                    "goal_normalizer_std": self.goal_normalizer.std}, "./savelib/saved_t.pth")

    def load_weights(self):

        checkpoint = torch.load("./savelib/saved_0103.pth")
