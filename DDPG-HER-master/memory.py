...
class Memory:
    ...
    def sample(self, batch_size):
        ...
        # print(rewards)
        #rewards = self.env.compute_reward(next_achieved_goals,desired_goals,None)
        #rewards = rewards.astype(np.float32) 
        # print(self.clip_obs(states), actions, rewards, self.clip_obs(next_states), self.clip_obs(desired_goals))
        return self.clip_obs(states), actions, rewards, self.clip_obs(next_states), self.clip_obs(desired_goals)

    def add(self, transition):
        self.memory.append(transition)
        # print("len", len(self.memory),"/",self.capacity)
